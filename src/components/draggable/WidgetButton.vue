        ...
        <el-form ref="form" label-position="right" label-width="120px">
            ...
            >
                <div
                    ...
                    :class="{
                        'active-main-item':
                            $store.state.location.type === 'buttons' && $store.state.location.value == index
                    }"
                >
...
export default {
    ...
    methods: {
        ...
        handleSelectWidget(item, index) {
            this.$store.commit("setShowConfigurationProperties", "buttons");
            this.$store.commit("setLocation", { type: "buttons", value: index });
            this.$store.commit("setConProPertiesButtons", item);
        },
        delComPerties(index) {
            this.$store.commit("setShowConfigurationProperties", "buttons");
            this.$store.commit("setLocation", { type: "buttons", value: index });
